================================================================================
    MFC (MICROSOFT FOUNDATION CLASS) ライブラリ : SharedMemoryTestVC プロジェクトの概要
===============================================================================

この SharedMemoryTestVC アプリケーションは、アプリケーション ウィザードによって作成されました。このアプリケーションは MFC の基本的な使い方を示すだけでなく、アプリケーションを作成するための開始点でもあります。

このファイルには、SharedMemoryTestVC アプリケーションを構成する各ファイルの内容の概要が含まれています。

SharedMemoryTestVC.vcxproj
...
SharedMemoryTestVC.vcxproj.filters
...
SharedMemoryTestVC.h
    これはアプリケーションのメイン ヘッダー ファイルです。
    その他のプロジェクト固有のヘッダー (Resource.h など) が含まれ、CSharedMemoryTestVCApp アプリケーション クラスを宣言します。

SharedMemoryTestVC.cpp
    これはメインのアプリケーション ソース ファイルで、CSharedMemoryTestVCApp アプリケーション クラスが含まれています。

SharedMemoryTestVC.rc
...
res\SharedMemoryTestVC.ico
    これはアプリケーションのアイコンとして使用されるアイコン ファイルです。このアイコンは、メイン リソース ファイル SharedMemoryTestVC.rc にインクルードされます。

res\SharedMemoryTestVC.rc2
...
SharedMemoryTestVCDlg.h, SharedMemoryTestVCDlg.cpp - ダイアログ
    これらのファイルには、CSharedMemoryTestVCDlg クラスが含まれます。このクラスはアプリケーションのメイン ダイアログの動作を定義します。ダイアログのテンプレートは、Microsoft Visual C++ で編集できる SharedMemoryTestVC.rc に含まれています。
...
StdAfx.h, StdAfx.cpp
    これらのファイルは、SharedMemoryTestVC.pch という名前のプリコンパイル済みヘッダー (PCH) ファイルと、StdAfx.obj という名前のプリコンパイル済みの型ファイルをビルドするために使用されます。

Resource.h
    これは、新しいリソース ID を定義する標準のヘッダー ファイルです。このファイルの読み込みおよび更新は、Microsoft Visual C++ で行います。

SharedMemoryTestVC.manifest
